...
// ===========================================================================================================================
//						 Last modified
// Neha 12 August 2014: case 9 added i.e. recordStatistics9() which is a combination of case 5 and case 7 to output
// i.e. time, aster#, aster position, MTname, MT length, +end.X, +end.Y
// Neha October 2015, RECORD Statistics 10 => coordinates of al lcomplexes, Aster centering
// Neha Khetan, July 2018 , Rotation project

//---------------------------------------------------------------------------------------------------------
// ------------------      GLIDING ASSAY
// Neha Khetan , April 2015: Fixed errors of CA for Gliding Assay calculations for Kunalika Jain experiments
// Neha Khetan , April 2015: Output the MT coordinates in time for GA
// Neha Khetan , May 2016, : 1D - 2D solved fixed for Gliding assay 1D coordinates simulations !!!!! 
// Neha khetan , May 2016  : include the coordinates of MT center
// Neha Khetan,  Sept. 2018: recordGhStatistics4() - Associate the forces exerted by motors on MTs
// ============================================================================================================================
 ...
 if ( IOGhStat.getOutputFile() == 0 ) {
  ...
  try {
    ...
    switch( MP.ghstatistics[0] ) {
      ...
      case 2:    recordGhStatistics2(); break;
      case 3:    recordGhStatistics3(); break;
      case 4:    recordGhStatistics4(); break;
...
// --------------------------------------------------------------------------------------------------
// Outputs the coordinates of all the motors ( Grafted ) in the system
void SIM::recordGhStatistics2()
{
  static bool virgin = true;
  if ( virgin ) 
    {
      virgin = false;
      IOGhStat.writeString("%% RecordGhStatistics2() Coordinates of all the grafted motors in system\n");
      IOGhStat.writeString("%% time |  GH name |  GH state | X(+) | Y(+) |  \n");
    }
  
  for( Grafted * ghi = firstBoundGrafted(); ghi ; ghi=ghi->next() )
    {

      IOGhStat.writeReal32( simTime() );
      IOGhStat.writeFormattingSpace(3);
      IOGhStat.writeReal32( ghi->getName() );
      IOGhStat.writeFormattingSpace(3);
      IOGhStat.writeReal32( ghi->getState() );
      IOGhStat.writeFormattingSpace(1);
      Vecteur pos = ghi->getPosition();
      IOGhStat.writeReal32Vect( DIM, pos );
      IOGhStat.writeFormattingNewLine();
    }
  // --------------------------------------------------------------------
  for( Grafted * ghi = firstFreeGrafted(); ghi ; ghi=ghi->next() )
    {
...
// ============================== GhStatistics3 =================================
/* CORTICAL MOTORS
1. 4 July 2018, Approach 3: Output at every time the bound motor, the corresponding MT ID, Aster ID, plus tip X and plus tip Y
2. Adding motor force - 17/10/2018
3. 20 october: Ensuring the output corresponds to only the cortical motors
*/
// Time| GHLoad| M1T id | MT length | MT Shrinkng? | Aster ID \n")7// 


void SIM::recordGhStatistics3()
{
  ...
  if ( virgin ) 
    {
      virgin = false;
      IOGhStat.writeString("% recordGhStatistics3(): July 2018, Associate bound cortical dynein motor with MTs and Asters\n");
      IOGhStat.writeString("% Modified October 20 October, 2018.\n");
      IOGhStat.writeString("% Time| GHLoad| MT id | MT length | MT Shrinkng? | Aster ID \n"); 
      // Time | MT id | MT len |  MT plus coords | Aster id | Aster coors | GH ID | GH LOad   
    }

  Microtub *tmpMT = 0;
  Aster *tmpAs = 0;

  // MT to which the motor is bound to
  Grafted * ghi;
  for( ghi = firstBoundGrafted(); ghi ; ghi=ghi->next() )
	{
    
		if ( ghi-> getType() == 0 )   // EXPLICITLY HARD-WIRED FOR CORTICAL DYNEINS, 20 October 2018
			{ 
		    IOGhStat.writeReal32( simTime());
		    IOGhStat.writeFormattingSpace(4); 

		    // Skipping the information on Motor ID, as this eats up space &
		    // Not possible to follow a single motor in time
		    // Thus this information is not essential, 20 October 2018
		    //IOGhStat.writeUInt16( ghi->getName());          // write Motor ID to which motor is bound 
		    //IOGhStat.writeFormattingSpace(3);

		    // Adding motor force - 17/10/2018
		    Vecteur stretch , dirmt;
		    stretch = ghi->getStretch();
		    dirmt   = ghi->getHand().dirMT();
		    Microtub *mt1 = ghi->getHand().getMT();         // Get MT ID to which motor is bound
		    
        IOGhStat.writeUInt16( mt1->getName() );         // Write MT ID
		    IOGhStat.writeFormattingSpace(4);  
		    IOGhStat.writeUInt16( mt1->length() ); 
		    IOGhStat.writeFormattingSpace(4); 

		    // 1 => SHRINKING ; 0 => GROWING 
		    int state = 0;
			  MTDynamicState state_now = mt1->getDynamicState( MT_PLUS_END );
		        if( state_now == MT_SHRINKING ) {
		    		state  = 1 ;
		    	};
			 IOGhStat.writeUInt16( state ); 
		   IOGhStat.writeFormattingSpace(4); 
       IOGhStat.writeReal32( mt1->whereEnd(MT_PLUS_END).XX);
       IOGhStat.writeFormattingSpace(4);
       IOGhStat.writeReal32( mt1->whereEnd(MT_PLUS_END).YY);
       IOGhStat.writeFormattingSpace(4);


		    // MT belongs to which Aster 
		    // Saving the time by checking if it exists
		    if( mt1 != tmpMT )

		      {
		        for( Aster* as1 = firstAster(); as1; as1=as1->next() ){
		          if ( mt1->getOrganizer() == as1 ){
		              tmpAs = as1;
		              IOGhStat.writeReal32( as1->getName() );     
                  IOGhStat.writeFormattingSpace(4);
                  IOGhStat.writeReal32Vect( DIM, as1->getPosition() );
                  IOGhStat.writeFormattingSpace(4);  
		              }
		            }
  		    }else{
        ...
        IOGhStat.writeUInt16( ghi->getName()); 
        IOGhStat.writeFormattingSpace(4);
        IOGhStat.writeReal32( ( stretch*dirmt ) * MP.km ); // Load force   
        IOGhStat.writeFormattingNewLine(); 
		  } 
	}
}
...
// ------------------------------------------------------------------------------------------------
// 12 September 2018, For Gliding assay and for next
// Output at every time the bound motor, the corresponding MT ID, MT length and the force exerted by that motor
void SIM::recordGhStatistics4()
{
  static bool virgin = true;
  if ( virgin ) 
    {
      virgin = false;
      IOGhStat.writeString("%% recordGhStatistics4(): September 2018 \n");
      IOGhStat.writeString("%% Time  | MT name  | MT length | Force  \n"); 
    }
    // Going over the bound motors list to find the MT to which it is bound
    Grafted * ghi;
    for( ghi = firstBoundGrafted(); ghi ; ghi=ghi->next() )
    {  
     
      IOGhStat.writeUInt16( simTime());
      IOGhStat.writeFormattingSpace(3);    
     
      Microtub *mt = ghi->getHand().getMT();         // Get MT ID to which motor is bound
      IOGhStat.writeUInt16( mt->getName() );         // Write MT ID
      IOGhStat.writeFormattingSpace(3); 
     
      IOGhStat.writeUInt16( mt->length() ); 
      IOGhStat.writeFormattingSpace(3); 
      ...
      dirmt   = ghi->getHand().dirMT();
      IOGhStat.writeReal32( ( stretch*dirmt ) * MP.km ); // Load force
      //printf( "%3.2f \t %d \t  %2.1f \t %10.4f \n" , simTime(),  mt->getName(),  mt->length() , ( stretch*dirmt ) * MP.km );
      IOGhStat.writeFormattingNewLine(); 
